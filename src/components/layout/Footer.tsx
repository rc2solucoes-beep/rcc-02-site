    ...
    <footer className="bg-rc2-forest text-rc2-sand">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        ...
        <div className="mt-12 pt-6 border-t border-rc2-sand/10 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="rc2-caption text-rc2-sand/80 text-center sm:text-left">
          ...
          <div className="flex gap-6">
            {legalLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="ui-focus-ring rounded-sm rc2-caption text-rc2-sand/80 hover:text-rc2-sand transition-colors"
              >
                {link.label}
              </Link>
            ))}
